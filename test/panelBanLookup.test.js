...
test('shows every returned ban, permanent and nickname restrictions', () => {
  ...
  assert.match(reply.embeds[0].toJSON().fields[0].value, /не блокировка аккаунта/);
  assert.equal(reply.embeds[0].toJSON().fields[1].value, 'Бессрочно');
